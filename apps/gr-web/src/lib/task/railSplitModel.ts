...
export const RAIL_TABS = ['split', 'layout', 'stock', 'cell', 'station', 'item'] as const
export type RailTab = (typeof RAIL_TABS)[number]

/** 나눠 보기의 표 쪽 토글. */
export const SPLIT_TABLES = ['cell', 'station', 'stock', 'item'] as const
...
export const DEFAULT_SPLIT: SplitState = { ratio: 0.55, orient: 'stack', table: 'cell' }
...
export function parseSplit(raw: string | null | undefined): SplitState {
  ...
  return {
    ...
    table:
      typeof o.table === 'string' && (SPLIT_TABLES as readonly string[]).includes(o.table)
        ? (o.table as SplitTable)
        : DEFAULT_SPLIT.table,
  }
}

export function serializeSplit(s: SplitState): string {
  return JSON.stringify({ ratio: clampRatio(s.ratio), orient: s.orient, table: s.table })
}
...
/**
 * 맵에서 셀·스테이션을 눌렀을 때 표 토글을 어디로 옮기나. 재고 표는 셀 단위라 셀을 누르면
 * 그대로 둔다(보던 재고 표를 빼앗지 않는다).
 */
export function tableForPick(current: SplitTable, kind: 'cell' | 'station'): SplitTable {
  if (kind === 'cell') return current === 'stock' || current === 'cell' ? current : 'cell'
  return 'station'
}
