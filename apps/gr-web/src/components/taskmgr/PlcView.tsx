...
function Ring({ title, rows, onPickKey, highlight, tone }: { title: string; rows: PlcTask[]; onPickKey?: PlcViewProps['onPickKey']; highlight: PlcViewProps['highlight']; tone: string }) {
  const filled = rows.filter((t) => !isZero(t)).length
  return (
    <div>
      <div className="mb-1 flex items-center gap-1.5">
        <StatusDot status={tone} size="sm" />
        <span className="text-2xs font-semibold tracking-wide text-content-muted uppercase">{title}</span>
      ...
      <table className="w-full text-[11px]" data-testid={`plc-ring-${title.toLowerCase()}`}>
        ...
        <tbody>
          {rows.map((t, i) => {
            const zero = isZero(t)
            const hit = !!highlight && !zero && t.WorkId === highlight.work_id && t.TaskId === highlight.task_id
            return (
              <tr
                key={i}
                className={cn(
...
export function PlcView({ onPickKey, highlight = null }: PlcViewProps) {
  ...
  const bits: FieldItem[] = [
    ...
    { label: 'Inprogress', value: s.Inprogress ? 'TRUE' : 'FALSE', status: s.Inprogress ? 'ok' : undefined },
  ...
  const r = data.reject
  const res: FieldItem[] = [
    { label: 'RES Header', value: h.CMD_ID || h.SEQ ? `CMD_ID ${h.CMD_ID} · SEQ ${h.SEQ} · CMD 0x${h.CMD.toString(16).toUpperCase()}` : null, mono: true, wide: true, missing: '에코 없음' },
    { label: '검증', value: `${r.validation} ${r.validation_text}`, status: r.validation === 1 ? 'ok' : r.validation === 0 ? undefined : 'fault' },
    { label: '거부 비트', value: [r.wrong_robot && 'WRONG_ROBOT', r.duplicate && 'DUPLICATE', r.invalid_task && 'INVALID', r.buffer_full && 'BUFFER_FULL', r.offline && 'OFFLINE'].filter(Boolean).join(', ') || null, status: 'fault', missing: '거부 비트 없음' },
    { label: 'Complete 허용', value: r.complete_allowed ? 'YES' : 'NO', status: r.complete_allowed ? 'ok' : 'warn' },
    { label: 'Delete 허용', value: r.delete_allowed ? 'YES' : 'NO', status: r.delete_allowed ? 'ok' : 'warn' },
  ]

  return (
    <div className="space-y-3 p-3" data-testid="plc-view">
      {error ? <p className="m-0 text-[11px] text-amber-600 dark:text-amber-400">마지막 읽기 실패 — {error}</p> : null}
      <div>
        <div className="mb-1 text-2xs font-semibold tracking-wide text-content-muted uppercase">Task.Status</div>
        <FieldList items={bits} columns={2} dense labelWidth={72} />
      </div>
      <div>
        <div className="mb-1 text-2xs font-semibold tracking-wide text-content-muted uppercase">RES</div>
        <FieldList items={res} columns={1} dense labelWidth={96} />
      </div>
      <Ring title="Now" rows={[data.now]} onPickKey={onPickKey} highlight={highlight} tone={isZero(data.now) ? 'neutral' : 'ok'} />
      <Ring title="Queue" rows={data.queue} onPickKey={onPickKey} highlight={highlight} tone="info" />
      <Ring title="Completed" rows={data.completed} onPickKey={onPickKey} highlight={highlight} tone="ok" />
      <Ring title="Canceled" rows={data.canceled} onPickKey={onPickKey} highlight={highlight} tone="neutral" />
      <Ring title="Rejected" rows={data.rejected} onPickKey={onPickKey} highlight={highlight} tone="fault" />
    </div>
  )
}
