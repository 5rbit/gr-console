...
import type { Cell, Defaults, Gate, Item, Station, Task, TaskRequest, TaskType } from '../../lib/types'
...
export function ComposeCard({ items, cells, stations, defaults, gate, onOpenDefaults }: ComposeCardProps) {
  ...
  const base = useMemo(() => effectiveParams(defaults, draft.type, kind, {}), [defaults, draft.type, kind])
    ...
    const tick = async () => {
      try {
        ...
        if (t.ack || TERMINAL.has(t.state)) {
          ...
          toast[t.ack?.accepted === false ? 'warn' : 'ok'](t.ack?.accepted === false ? `#${t.seq} 거부됨 (코드 ${t.ack.code})` : `#${t.seq} ${t.ack ? '수락됨' : t.state}`)
          return
        }
  ...
  return (
    <Card padded={false} className="flex flex-col" data-testid="compose-card">
      <div className="flex items-center gap-2 border-b border-slate-200 px-3 py-2 dark:border-slate-700">
        ...
        <Button size="sm" intent="ghost" icon={<Settings2 className="h-3.5 w-3.5" />} onClick={onOpenDefaults} data-testid="open-defaults">
          기본값
        </Button>
        <Button size="sm" intent="ghost" icon={<RotateCcw className="h-3.5 w-3.5" />} onClick={() => setDraft(EMPTY_DRAFT)} title="초안 비우기">
          초기화
        </Button>
      </div>
      ...
      <div className="flex flex-col gap-3 px-3 py-3">
        <div className="flex flex-wrap items-end gap-2">
          <Select label="작업 종류" value={draft.type} onValueChange={(v) => set({ type: v as TaskType })} data-testid="compose-type">
            {TASK_TYPES.map((t) => (
          ...
          <Input label="수량" type="number" mono min={1} max={255} step="1" className="w-20" value={String(draft.count)} onValueChange={(s) => set({ count: Number(s) })} data-testid="compose-count" />
          <Input label="메모" className="min-w-40 flex-1" value={draft.note} onValueChange={(note) => set({ note })} placeholder="원장에 남는 한 줄" />
        </div>

        <TargetPicker value={draft.target} onChange={(target) => set({ target })} cells={cells} stations={stations} kinds={kinds} />

        <div className="flex flex-wrap items-end gap-2">
          <ItemPicker value={draft.item_code} onChange={(item_code) => set({ item_code })} items={items} allowNone={!itemRequired(draft.type)} />
          {!targetRequired(draft.type) ? <span className="pb-2 text-xs text-slate-400">UP은 대상 없이 제출할 수 있습니다</span> : null}
        </div>

        <div className="rounded-md border border-slate-200 dark:border-slate-700">
          <button type="button" className="flex w-full items-center gap-2 px-2 py-1.5 text-left text-xs font-medium" onClick={() => setShowParams((s) => !s)} aria-expanded={showParams} data-testid="toggle-params">
            {showParams ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
            파라미터 덮어쓰기
            <span className="font-normal text-slate-400">
              {overrideCount ? `${overrideCount}개 덮어씀` : '모두 기본값 사용'} · 기본값 = 공통 ← {draft.type}·{kind === 'cell' ? '셀' : '스테이션'}
            </span>
            <span className="flex-1" />
            {overrideCount ? (
              <Button
          ...
          {showParams ? (
            <div className="border-t border-slate-200 px-2 py-2 dark:border-slate-700">
              <p className="mb-2 text-[11px] text-slate-500">체크를 끄면 그 항목은 기본값을 씁니다(흐린 값). 켠 항목만 이 작업에 실립니다.</p>
              <TaskParamFields value={draft.params} base={base} onChange={(params) => set({ params })} partial />
            </div>
          ) : null}
        </div>
        ...
        {ack ? <AckBanner task={ack.task} phase={ack.phase} onDismiss={() => setAck(null)} /> : null}

        <div className="flex items-center justify-end gap-2">
          <Button intent="primary" icon={<Send className="h-4 w-4" />} disabled={!canSubmit} loading={submitting} onClick={() => setConfirm(true)} data-testid="compose-submit" title={!gate?.can_submit ? '게이트가 닫혀 있습니다' : request ? '' : '초안이 완성되지 않았습니다'}>
            제출
          </Button>
        </div>
      </div>

      <ConfirmDialog open={confirm} onOpenChange={setConfirm} scope="single-robot" danger title={`${draft.type} 작업 제출`} confirmLabel="PLC로 제출" onConfirm={() => void submit()}>
        <div className="flex flex-col gap-1 text-xs">
          <p>
            <b>{draft.type}</b>
            {draft.target ? ` · ${draft.target.kind === 'cell' ? '셀' : '스테이션'} #${draft.target.id}` : ' · 대상 없음'}
            {draft.item_code !== null ? ` · 품목 ${draft.item_code} × ${draft.count}` : ''}
          </p>
          {preview ? (
            <p className="font-mono text-slate-500">
              X {preview.task.Position[0]} · Y {preview.task.Position[1]} · Z {preview.task.Position[2]} · G {preview.task.Position[3]}
            </p>
          ) : null}
          {preview && preview.warnings.length ? <p className="text-amber-700 dark:text-amber-300">경고 {preview.warnings.length}건 — 미리보기를 확인하세요</p> : null}
          <p className="text-slate-500">GRM OPC UA를 거쳐 GR2에 명령이 갑니다. 로봇이 실제로 움직입니다.</p>
        </div>
      </ConfirmDialog>
    </Card>
  )
