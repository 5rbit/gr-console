...
export const PLC_TARGET_LABEL: Record<PlcTarget, string> = { GR2: 'GR2', GRM: 'GRM', both: 'GR2 + GRM' }
...
export function PushDialog({ open, onOpenChange, what, plc, rows, dirty, onConfirm }: PushDialogProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 text-xs">
        <p>
          로컬 {what} <b>{rows}</b>건(수정 {dirty}건)을 Id 오름차순으로 PLC DB에 쓰고 나머지 슬롯은 0으로 채웁니다.
          <b> Count</b>는 마지막에 씁니다. 쓴 뒤 다시 읽어 바이트를 대조합니다.
        </p>
        <p className="text-slate-500">PLC의 기존 테이블은 되돌릴 수 없습니다 — 먼저 <b>PLC 읽기</b>로 백업하거나 Excel로 내보내 두세요.</p>
        <label className="flex items-center gap-2">
          <input type="checkbox" checked={force} onChange={(e) => setForce(e.currentTarget.checked)} data-testid="push-force" />
...
export function DiffDialog<T>({ open, onOpenChange, title, rows, error, summarize }: DiffDialogProps<T>) {
  ...
  const columns: Column<DiffRow<T>>[] = [
    ...
    { key: 'status', label: '상태', get: (r) => r.status, cell: (r) => <DiffBadge status={r.status} /> },
    { key: 'local', label: '로컬', get: (r) => (r.local ? summarize(r.local) : null), cell: (r) => (r.local ? <span className="font-mono text-[11px]">{summarize(r.local)}</span> : <span className="text-slate-400">—</span>) },
    { key: 'plc', label: 'PLC', get: (r) => (r.plc ? summarize(r.plc) : null), cell: (r) => (r.plc ? <span className="font-mono text-[11px]">{summarize(r.plc)}</span> : <span className="text-slate-400">—</span>) },
  ]
  return (
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <label className="flex items-center gap-1">
            <input type="checkbox" checked={onlyDiff} onChange={(e) => setOnlyDiff(e.currentTarget.checked)} />
            다른 것만
          </label>
        </div>
        {error ? <p className="text-xs text-red-600">{error}</p> : null}
        <DataTable rows={shown} columns={columns} rowKey={(r) => String(r.id)} loading={rows === null && !error} empty={onlyDiff ? '차이 없음 — 로컬과 PLC가 같습니다' : '행 없음'} testid="diff-table" />
      </div>
    </Modal>
  )
}
...
export function ImportDialog({ open, onOpenChange, file, preview, error, applying, onApply }: ImportDialogProps) {
    ...
    <Modal open={open} onOpenChange={onOpenChange} title="Excel 가져오기 — 미리보기" wide>
      <div className="flex flex-col gap-3 text-xs">
        <p className="text-slate-500">
          ...
          {preview?.counts ? ` · 셀 ${preview.counts.cells} · 스테이션 ${preview.counts.stations} · 품목 ${preview.counts.items}` : ''}
        </p>
        ...
        {preview ? (
          <div className="grid grid-cols-4 gap-2" data-testid="import-preview">
            {[
              ...
              <div key={String(l)} className="rounded-md border border-slate-200 px-2 py-1 dark:border-slate-700">
        ...
        {preview && preview.errors.length > 0 ? (
          <DataTable rows={preview.errors} columns={errCols} rowKey={(e) => `${e.sheet ?? ''}:${e.row}:${e.message}`} testid="import-errors" />
        ) : null}
        <p className="text-slate-500">오류 행은 건너뛰고 나머지만 로컬에 적용합니다(PLC에는 쓰지 않습니다 — 적용 뒤 <b>PLC 쓰기</b>로 반영).</p>
        <div className="flex justify-end">
          <Button intent="primary" size="sm" icon={<Upload className="h-3.5 w-3.5" />} disabled={!canApply} loading={applying} onClick={onApply} data-testid="import-apply">
            적용
          </Button>
        </div>
      </div>
