// 스테이션 그리드 편집기(엑셀형) — 레이아웃 편집 모드 우측 사이드바. 셀 편집기와 같은 규칙:
// 칸 편집 · 엑셀 붙여넣기 · 행 추가/삭제 · 되돌리기 · 한 번에 적용(로컬 사본), 저장 전 초안은 맵에 바로 그린다.
// 스테이션 좌표는 PLC isValid_Station_Parameter 가 X/Y/Z ≤ 0 을 거부하므로 셋 다 0 보다 커야 한다.
import { useEffect, useMemo, useRef, useState } from 'react'
import { Box, Plus, Save, Trash2, Undo2 } from 'lucide-react'
import { api } from '../../lib/api'
import type { Registry } from '../../lib/registry'
import { taskApi } from '../../lib/task/api'
import {
  applyStationEdit,
  boolText,
  diffDraft,
  draftFrom,
  isStationIdOk,
  newStationRow,
  pasteBool,
  pasteInt,
  pasteNum,
  savedKey,
  setRow,
  stationToUpsert,
  type DraftRow,
} from '../../lib/task/registryGrid'
import { Button } from '../../lib/ui/Button'
import { ConfirmDialog } from '../../lib/ui/ConfirmDialog'
import { DataGrid, type DataGridColumn } from '../../lib/ui/datagrid/DataGrid'
import { toast } from '../../lib/ui/toast'
import type { Station, StationUpsert } from '../../lib/types'
import { EMPTY_STATION } from './forms'
import { RegistryToolbar, type RegistryIo } from './RegistryToolbar'
import { matchStation, stationSummary } from './StationRegistry'

type Row = DraftRow<StationUpsert>

const IO: RegistryIo<Station> = {
  plc: {
    import: taskApi.stationsImport,
    push: taskApi.stationsPush,
    diff: taskApi.stationsDiff,
    summarize: stationSummary,
  },
  exportUrl: taskApi.stationsExportUrl,
  importFile: taskApi.stationsImportFile,
}

export interface StationGridEditorProps {
  reg: Registry<Station>
  q: string
  selectedId: number | null
  onSelect: (id: number | null) => void
  /** 저장 전 초안 — 맵 미리보기용. 편집기가 사라지면 null. */
  onDraft: (stations: Station[] | null) => void
}

export function StationGridEditor({
  reg,
  q,
  selectedId,
  onSelect,
  onDraft,
}: StationGridEditorProps) {
  const saved = reg.items
  // 초안이 어느 저장본에서 나왔는지 — 첫 로드·PLC 읽기로 목록이 바뀌어도 "전부 삭제" 로 오인하지 않게 이것과 비교한다.
  const [base, setBase] = useState<readonly Station[]>(saved)
  const savedById = useMemo(() => new Map(saved.map((s) => [s.id, s])), [saved])
  const original = useMemo(
    () => new Map(base.map((s) => [savedKey(s.id), stationToUpsert(s)])),
    [base],
  )
  const [rows, setRows] = useState<Row[]>(() => draftFrom(saved.map(stationToUpsert)))
  const [selKey, setSelKey] = useState<string | null>(null)
  const [busy, setBusy] = useState(false)
  const [confirm, setConfirm] = useState(false)
  const [stale, setStale] = useState(false)
  const justSaved = useRef(false)
  const diff = useMemo(
    () =>
      diffDraft(
        rows,
        base.map((s) => s.id),
      ),
    [rows, base],
  )
  const pending = diff.upserts.length + diff.deletes.length
  const pendingRef = useRef(pending)
  pendingRef.current = pending

  useEffect(() => {
    if (saved === base) return
    if (pendingRef.current === 0 || justSaved.current) {
      setBase(saved)
      setRows(draftFrom(saved.map(stationToUpsert)))
      setStale(false)
      justSaved.current = false
    } else {
      setStale(true)
    }
  }, [saved])

  useEffect(() => {
    onDraft(
      rows.map((r) => {
        const s = savedById.get(r.value.id)
        return {
          ...r.value,
          source: s?.source ?? 'local',
          dirty: r.state !== 'same' || (s?.dirty ?? true),
          updated_at: s?.updated_at ?? '',
        }
      }),
    )
  }, [rows, savedById, onDraft])
  useEffect(() => () => onDraft(null), [onDraft])

  useEffect(() => {
    if (selectedId === null) return
    const r = rows.find((x) => x.value.id === selectedId)
    if (r) setSelKey(r.key)
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 선택 변경에만 반응
  }, [selectedId])

  const dup = useMemo(() => new Set(diff.dupIds), [diff.dupIds])
  const idError = (s: StationUpsert) =>
    dup.has(s.id)
      ? `id ${s.id} 중복`
      : isStationIdOk(s.id)
        ? null
        : '스테이션 id 는 2001..2999, id mod 100 = 1..32'
  const sectionError = (s: StationUpsert) =>
    [1, 2, 3].includes(s.info.section) ? null : '구역은 1..3'
  const posError = (s: StationUpsert, i: 0 | 1 | 2) =>
    s.info.position[i] > 0 ? null : `위치 ${'XYZ'[i]}는 0보다 커야 합니다 (PLC 스테이션 검증)`
  const errorCount = rows.filter(
    (r) =>
      idError(r.value) ||
      sectionError(r.value) ||
      [0, 1, 2].some((i) => posError(r.value, i as 0 | 1 | 2)),
  ).length

  const shown = useMemo(() => {
    const needle = q.trim()
    if (!needle) return rows
    return rows.filter((r) =>
      matchStation({ ...r.value, source: 'local', dirty: false, updated_at: '' }, needle),
    )
  }, [rows, q])

  const stateLabel = (r: Row) =>
    r.state === 'added'
      ? '신규'
      : r.state === 'changed'
        ? '수정'
        : savedById.get(r.value.id)?.dirty
          ? '로컬'
          : 'PLC'
  const num = (
    id: string,
    header: string,
    get: (s: StationUpsert) => number,
    extra: Partial<DataGridColumn<Row>> = {},
  ): DataGridColumn<Row> => ({
    id,
    header,
    width: '3rem',
    align: 'right',
    mono: true,
    editor: 'number',
    decimals: 0,
    text: (r) => String(get(r.value)),
    coercePaste: pasteInt,
    ...extra,
  })
  const columns: DataGridColumn<Row>[] = [
    {
      id: 'state',
      header: '',
      width: '2.5rem',
      editor: 'none',
      sticky: true,
      text: stateLabel,
      cellClass: (r) =>
        r.state === 'added'
          ? 'font-semibold text-ok-fg'
          : r.state === 'changed'
            ? 'font-semibold text-warn-fg'
            : 'text-content-faint',
    },
    num('id', 'Id', (s) => s.id, {
      width: '3.5rem',
      sticky: true,
      invalid: (r) => idError(r.value),
    }),
    num('conv', 'CV', (s) => s.conv_no),
    num('group', '그룹', (s) => s.group),
    num('group_index', '순번', (s) => s.group_index),
    num('x', 'X', (s) => s.info.position[0], {
      width: '4.5rem',
      decimals: 1,
      coercePaste: (v) => pasteNum(v),
      invalid: (r) => posError(r.value, 0),
    }),
    num('y', 'Y', (s) => s.info.position[1], {
      width: '4.5rem',
      decimals: 1,
      coercePaste: (v) => pasteNum(v),
      invalid: (r) => posError(r.value, 1),
    }),
    num('z', 'Z', (s) => s.info.position[2], {
      width: '4.5rem',
      decimals: 1,
      coercePaste: (v) => pasteNum(v),
      invalid: (r) => posError(r.value, 2),
    }),
    {
      id: 'section',
      header: '구역',
      width: '2.75rem',
      align: 'center',
      editor: 'select',
      options: ['1', '2', '3'],
      text: (r) => String(r.value.info.section),
      invalid: (r) => sectionError(r.value),
      coercePaste: pasteInt,
    },
    num('row', '행(X)', (s) => s.info.row),
    num('col', '열(Y)', (s) => s.info.col),
    {
      id: 'use',
      header: '사용',
      width: '2.75rem',
      align: 'center',
      editor: 'select',
      options: ['Y', 'N'],
      text: (r) => boolText(r.value.info.use),
      coercePaste: pasteBool,
    },
    num('task_type', '작업', (s) => s.task_type),
    num('rotate_type', '회전', (s) => s.rotate_type),
  ]

  function applyOne(cur: Row[], key: string, col: string, value: string): Row[] {
    const r = cur.find((x) => x.key === key)
    if (!r) return cur
    const next = applyStationEdit(r.value, col, value)
    return next ? setRow(cur, key, next, original) : cur
  }
  const edit = (key: string, col: string, value: string) =>
    setRows((cur) => applyOne(cur, key, col, value))
  const editMany = (ups: { rowId: string; colId: string; value: string }[]) =>
    setRows((cur) => ups.reduce((acc, u) => applyOne(acc, u.rowId, u.colId, u.value), cur))

  function addRow() {
    const r = newStationRow(rows, EMPTY_STATION)
    if (!r) {
      toast.warn('남은 스테이션 id 가 없습니다 (2001..2999)')
      return
    }
    setRows((cur) => [...cur, r])
    setSelKey(r.key)
    onSelect(r.value.id)
  }
  function removeSelected() {
    if (!selKey) return
    setRows((cur) => cur.filter((r) => r.key !== selKey))
    setSelKey(null)
    onSelect(null)
  }
  function revert() {
    setBase(saved)
    setRows(draftFrom(saved.map(stationToUpsert)))
    setStale(false)
  }
  async function save() {
    setBusy(true)
    try {
      for (const id of diff.deletes) await api.stationDelete(id)
      for (const s of diff.upserts) {
        if (savedById.has(s.id)) await api.stationUpdate(s.id, s)
        else await api.stationCreate(s)
      }
      justSaved.current = true
      await reg.reload()
      toast.ok(
        `스테이션 적용 — 수정 ${diff.changed} · 신규 ${diff.added} · 삭제 ${diff.deletes.length} (로컬 사본, PLC 반영은 PLC 쓰기)`,
      )
    } catch (e) {
      toast.error(`스테이션 적용 실패 — ${e instanceof Error ? e.message : String(e)}`)
    } finally {
      setBusy(false)
    }
  }

  return (
    <div className="flex h-full min-h-0 flex-col" data-testid="station-grid-editor">
      <RegistryToolbar<Station>
        compact
        hideCrud
        title="스테이션"
        icon={<Box size={14} />}
        what="스테이션"
        rows={saved.length}
        dirty={saved.filter((s) => s.dirty).length}
        selected={false}
        io={IO}
        onAdd={addRow}
        onEdit={() => undefined}
        onDelete={removeSelected}
        reload={reg.reload}
      />
      <div className="flex h-10 flex-none items-center gap-1 border-b border-line-default px-2">
        <Button
          size="sm"
          intent="ghost"
          icon={<Plus className="h-3.5 w-3.5" />}
          onClick={addRow}
          data-testid="station-grid-add"
        >
          행 추가
        </Button>
        <Button
          size="sm"
          intent="ghost"
          icon={<Trash2 className="h-3.5 w-3.5" />}
          disabled={!selKey}
          onClick={removeSelected}
          data-testid="station-grid-delete"
        >
          행 삭제
        </Button>
        <span className="min-w-0 flex-1 truncate px-1 text-2xs text-content-muted tabular-nums">
          {pending
            ? `수정 ${diff.changed} · 신규 ${diff.added} · 삭제 ${diff.deletes.length}`
            : '칸을 바로 고치거나 엑셀에서 붙여넣기'}
          {errorCount ? <span className="ml-1 text-fault-fg">· 오류 {errorCount}</span> : null}
        </span>
        <Button
          size="sm"
          intent="ghost"
          icon={<Undo2 className="h-3.5 w-3.5" />}
          disabled={!pending || busy}
          onClick={revert}
          data-testid="station-grid-revert"
        >
          되돌리기
        </Button>
        <Button
          size="sm"
          intent="primary"
          icon={<Save className="h-3.5 w-3.5" />}
          disabled={!pending || errorCount > 0 || busy}
          loading={busy}
          onClick={() => setConfirm(true)}
          data-testid="station-grid-apply"
        >
          적용{pending ? ` ${pending}` : ''}
        </Button>
      </div>
      {stale ? (
        <div className="flex h-8 flex-none items-center border-b border-warn bg-warn-soft px-3 text-2xs text-warn-fg">
          저장본이 바뀌었습니다(PLC 읽기·가져오기). 편집을 적용하거나 되돌리기로 새로 받으세요.
        </div>
      ) : null}
      <div className="min-h-0 flex-1">
        <DataGrid<Row>
          rows={shown}
          columns={columns}
          rowId={(r) => r.key}
          persistKey="layout-station-grid"
          layoutFixed
          zebra
          maxHeight="calc(100vh - 15rem)"
          empty="스테이션이 없습니다 — 행 추가 또는 엑셀에서 붙여넣기"
          rowClass={(r) => (r.key === selKey ? 'bg-accent-soft' : '')}
          onrowclick={(r) => {
            setSelKey(r.key)
            onSelect(r.value.id)
          }}
          onedit={edit}
          oneditmany={editMany}
          onpasteskipped={(n) => toast.warn(`붙여넣기 ${n}칸은 형식이 맞지 않아 건너뛰었습니다`)}
          cellTestId={(r, col) => `stationgrid-${r.value.id}-${col.id}`}
        />
      </div>
      <ConfirmDialog
        open={confirm}
        onOpenChange={setConfirm}
        scope="single"
        title="스테이션 편집 적용"
        confirmLabel="적용"
        onConfirm={() => void save()}
      >
        <p className="text-xs">
          수정 {diff.changed} · 신규 {diff.added} · 삭제 {diff.deletes.length} 건을 로컬 사본에
          저장합니다. PLC 반영은 툴바의 PLC 쓰기로 합니다.
        </p>
      </ConfirmDialog>
    </div>
  )
}
