  ...
  return (
    <div>
          ...
          <thead className="sticky top-0 bg-surface-panel">
            <tr className="text-content-muted">
              {HEAD.map((h, i) => (
                <th
                  key={h}
                  className={`px-1.5 py-1 ${i === 0 || i === 6 ? 'text-left' : 'text-right'}`}
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {slots.length === 0 ? (
              ...
              slots.map((b) => (
                <tr
                  key={b.Code}
                  className="cursor-pointer border-t border-line-default hover:bg-surface-app"
                ...
                >
                  ...
                  <td className="px-1.5 text-right font-mono tabular-nums">
                    {f1(b.Item?.InnerDiameter)}
                  </td>
                  <td className="px-1.5 text-right font-mono tabular-nums">
                    {f1(b.Item?.OuterDiameter)}
                  </td>
      ...
      <div className="mt-1 text-2xs text-content-muted">
        각 셀 = 평균 / EMA (최소 ~ 최대, n). 행을 클릭하면 해당 Code 로 필터됩니다.
      </div>
    </div>
  )
}
