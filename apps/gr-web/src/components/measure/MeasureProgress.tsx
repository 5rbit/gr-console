...
import { Chips, KvTable, StatCards } from './helpers'
    ...
    <div>
      <StatCards
        items={[
          ...
          { label: '콘솔 미러', value: (snap as unknown as { mirrored?: number } | null)?.mirrored ?? snap?.total ?? '' },
        ]}
      />
            ...
            rows={[
              [
                ...
                <span key="s" className="flex flex-wrap gap-1">
                  <Chips obj={mi} keys={['Busy', 'Done', 'Reported']} />
                  <StatusBadge status={num(mi, 'Status') === 3 || num(mi, 'Status') === 4 ? 'fault' : 'info'}>{STATUS[num(mi, 'Status') ?? 0] ?? num(mi, 'Status')}</StatusBadge>
                </span>,
              ],
              ['WorkCell / Code', `${num(mi, 'WorkCell') ?? ''} / ${num(mi, 'Code') ?? ''}`],
              ['들어갈 때', <span key="in" className="flex flex-wrap items-center gap-1"><Chips obj={mi} keys={['InBusy', 'InDone', 'InValid']} /> 내경 {f1(num(mi, 'InInnerDia'))}</span>],
              ['나갈 때', <span key="out" className="flex flex-wrap items-center gap-1"><Chips obj={mi} keys={['OutBusy', 'OutDone', 'OutValid']} /> 내경 {f1(num(mi, 'OutInnerDia'))}</span>],
              ['토크 내경', f1(num(mi, 'Torq_InnerDia'))],
              ['정합 내경', f1(num(mi, 'InnerDia'))],
              ['편심 X / Y', `${f2(num(mi, 'OffsetX'))} / ${f2(num(mi, 'OffsetY'))}`],
              ['상단 비드 / 타이어 높이', `${f1(num(mi, 'UpperBeadHeight'))} / ${f1(num(mi, 'TireHeight'))}`],
            ]}
          />
        </Card>
        <Card>
          ...
          <KvTable
            rows={[
              ['상태', <Chips key="s" obj={ms} keys={['Enable', 'Busy', 'Done']} />],
              ['WorkCell', num(ms, 'WorkCell') ?? ''],
              ['단수', num(ms, 'StackCount') ?? ''],
              ['단당 / 스택 높이', `${f1(num(ms, 'EachHeight'))} / ${f1(num(ms, 'StackHeight'))}`],
              ['비드 위치', ((ms?.BeadPos as number[] | undefined) ?? []).filter((v) => v).map(f1).join(', ') || '-'],
            ]}
          />
        </Card>
        <Card>
          <h3 className="mb-1 text-xs font-semibold text-content-muted">Floor / Pick 마지막 비드</h3>
          <KvTable
            rows={[
              ['Floor', <span key="f" className="flex flex-wrap items-center gap-1"><Chips obj={mf} keys={['Enable', 'Busy', 'Done']} /> 높이 {f1(num(mf, 'Height'))}</span>],
              ['Pick 마지막 비드', <Chips key="b" obj={mb} keys={['Enable', 'Busy', 'Done']} />],
              ['LastBidPos / PickZTarget', `${f1(num(mb, 'LastBidPos'))} / ${num(mb, 'PickZTarget') ? f1(num(mb, 'PickZTarget')) : '-'}`],
            ]}
          />
          <h3 className="mt-3 mb-1 text-xs font-semibold text-content-muted">OPCUA Measuring (보고 영역)</h3>
          <KvTable
            rows={[
              ['SKU', `Cell ${om?.SKU?.WorkCell ?? ''} Code ${om?.SKU?.Code ?? ''} Status ${STATUS[om?.SKU?.Data?.[0] ?? 0] ?? ''}`],
              ['Item', `Cell ${om?.Item?.WorkCell ?? ''} Code ${om?.Item?.Code ?? ''} Status ${STATUS[om?.Item?.Data?.[0] ?? 0] ?? ''} 내경 ${f1(om?.Item?.Data?.[1])}`],
