...
CREATE INDEX IF NOT EXISTS stock_log_key ON stock_log(kind, key, id);
-- 완료 반영 기록(Task 하나 = 한 줄, 정확히 한 번): applied(재고에 반영) | ignored(손 정정 뒤라 사람이 무시)
ALTER TABLE stock_applied ADD COLUMN outcome TEXT NOT NULL DEFAULT 'applied';
