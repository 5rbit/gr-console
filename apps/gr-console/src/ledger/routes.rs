//! `/api/tasks*` (M3-B slice refines; M1 provides the baseline the frontend contract needs).
...
use super::{LedgerEntry, Origin, TaskRequest, TaskState};
...
async fn list(State(st): State<AppState>, Query(q): Query<ListQuery>) -> ApiResult<Json> {
    ...
    let (items, total) = st.ledger.query(states, task_type, q.q, limit, offset)?;
...
async fn plc_view(State(st): State<AppState>) -> ApiResult<Json> {
    ...
    Ok(axum::Json(json!({ "status": v.task.status, "now": v.task.now, "queue": v.task.queue, "completed": v.task.completed, "canceled": v.task.canceled, "rejected": v.task.rejected, "res": v.res, "reject": v.reject_info() })))
}
