...
가중치와 파라미터도 팝업(도구 `⋯`). 자동 생성 스위치는 켤 때 한 번 되묻는다(끄기는 바로).

규칙 표의 한 줄이 곧 모니터다 — `상태`(칩) · `조건 값`(입력의 지금 값) · `만든 건`(누적, 툴팁에 마지막 시각).
상태 칩은 `꺼짐`(규칙 off) · `조건 대기`(조건 거짓) · `만들 수 있음`(조건 참 · 자동 꺼짐) · `생성` · `예정` ·
`진행 중` · `대기`(영역·짝) · `못 만듦`(자동 셀 없음 · 미등록 · 거리). 툴팁이 사유와 조건 값, 조건이 참이 된 뒤
지난 시간을 함께 말한다. 아래 모니터 묶음은 지표 한 줄 · 후보(점수 근거 툴팁) · 예정 큐다.
`GET /api/taskgen` 의 `rules[]`(`taskgen::RuleStatus`)가 이 줄의 재료다.
